...
class Damid(Restraint):
    ...
    def _apply_envelope(self, model):
        ...
        # loop over particles, and compute the distnace...if that is larger than activation distance, you need DamID restraint
        affected_particles = [
        ...
        f = model.addForce(
            EllipticEnvelope(
                ...
                center,
                np.array([self.a, self.b, self.c])*cutoff,
                -self.k,     # negative elastic constant, centrally repulsive
                scale = cutoff
            )
        )

...
class RelaxInit(StructGenStep):
    ...
    def before_map(self):
        ...
        # we run the poller here, which starts running in the background before the actual mapping happens
        self._run_poller()
    ...
    def task(struct_id, cfg, tmp_dir):
        ...
        if cfg['model']['restraints']['envelope']['nucleus_shape'] == 'sphere':
            ...
            # compute the resto intero
            env_idx = struct_id % 4

            logger.info("Index of envelope used = " + str(env_idx))
            ev = GenEnvelope(cfg['model']['restraints']['envelope']['nucleus_shape'],
                             cfg['model']['restraints']['envelope']['input_map'][env_idx],
                             cfg['model']['restraints']['envelope']['nucleus_kspring'])
    ...
    def reduce(self):
        """
        Collect all structure coordinates together to assemble a hssFile, AFTER the polling function was taken down, and repack
        """

...
class SpriteAssignmentStep(Step):
    ...
    def setup(self):
        ...
        # read in SPRITE volume fraction, and the filename
        volume_fraction = self.cfg.get("runtime/sprite/volume_fraction")
        logger.info("SPRITE volume fraction = " + str(volume_fraction))
    ...
    def task(batch_id, cfg, tmp_dir):
        ...
        for cluster in clusters:
            ...
            # if max_chrom_in_cluster exceeeded, then append arrays full of -1 entries, there is no call of "compute_gyration-radius"
    ...
    def reduce(self):
        ...
        """ Concanenate all data into a single sprite_actdist_file """

        # build suffix to append to actdist file (code does not overwrite actdist files)
        additional_data = []
        if "sprite" in self.cfg['runtime']:
            additional_data.append(
                'volume_fraction_{:.4f}'.format(
                    self.cfg['runtime']['sprite']['volume_fraction']
                )
            )
            ...
            for batch_id in tqdm(random_order, desc='(REDUCE)'):
                ...
                for i, (best_rg2s, curr_idx) in enumerate(results):   # i is a counter
                    ...
                    if best_rg2s[0] < 0:    # negative radius of gyration squared (should not happen),
                                            # this means more than 6 diff chromosomes are involved in cluster (see IF lines 95-101)
                        pos = 0
                        si = -1
                    else:
                        ...
                        # this ensures that "struct_penalization" is positive, even if "occupancy" is zero
                        structure_penalizations = np.clip(occupancy[curr_idx] - aveN, 0., None) / stdN    # this should be an array... each structure in pool
                                                                                                          # comes with its penalization
                    ...
                    # si < -1, if cluster is too large... this is no actual structure index, so cluster not assigned (see also /restraints/sprite.py)
                    assignment[ci] = si

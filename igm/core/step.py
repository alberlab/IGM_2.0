...
class Step(object):
    def __init__(self, cfg):
        ...
        # Keep track of step execution in a database DB
    ...
    def task(struct_id, cfg, tmp_dir):
        """
        actual serial function that supposed to be passed to each  worker and executed there
        """
        pass
    ...
    def reduce(self):
        """
        Do something after parallel jobs: collect all info into master files
        """
    ...
    # the run attribute calls all the others above
    def run(self):
        """
        ...
        *DO NOT OVERLOAD*
        ...
        if 'completed' in past_substeps:
            ...
            # the full Step is complete, so call skip and move on
            self.skip()
            return
        ...
        try:
            ...
            # define the serial function (static method function) which acutally performs the mapping
            ...
            # the "mapping" step has not been run yet, so run "before_map()" and "map()"
            ...
            # if "mapping step" was run, then update database
            else:
                self.cfg['runtime'].update(past_substeps['mapped']['runtime'])

            # if "reduce" step has not been run, run "before_reduce()" and "reduce()"
            ...
            # if "reduce" ste was run, then update the database
            else:
                self.cfg['runtime'].update(past_substeps['reduced']['runtime'])

            # same for cleanup step: either run or update database
            ...
            # print completion infomration to logger

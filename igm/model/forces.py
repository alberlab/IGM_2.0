...
class EllipticEnvelope(Force):
    ...
    def __init__(self, particle_ids,
        ...
        self.scale = scale  # pff. This is to actually give a
                            # "relative  measure" for violation ratios, "1 - contact_range"
        self.note = note
    ...
    def getScores(self, particles):
        ...
        for q, i in enumerate(self.particle_ids):
            ...
            if self.k > 0:
               s2 = np.square(self.semiaxes - p.r)   # no need to use "cutoff" for volumetric restraints
            else:
               s2 = np.square(self.semiaxes - p.r)

            k2 = np.sum(x2 / s2)

            # if particle is outside of the envelope and we are talking envelope restraint
            if k2 > 1 and self.k > 0:    # if volume restraint
                t = ( 1.0 - 1.0/np.sqrt(k2) )*np.linalg.norm(x)/self.scale    # thsi reproduces guido's formulation exactly
                 
            # if particle is inside of envelope and we are talking lamina DamID restraint
            elif k2 < 1 and self.k < 0:  # if damid restraint
                t = ( 1.0 - np.sqrt(k2))   # relative distance of point to radial correspodning point on the ellipse at contact_range
            
            else:
                t = 0

            scores[q] = max(0, t)       # Guido had scores[k] = max(0, t * self.k)..in this formulation, t*k would be negative  all the times for DAMID
    ...
    def getViolationRatios(self, particles):
        return self.getScores(particles)                 # divide by (self.k)
